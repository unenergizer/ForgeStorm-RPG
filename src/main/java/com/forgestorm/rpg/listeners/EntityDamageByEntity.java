...
import com.forgestorm.rpg.combat.Fatigue;
import com.forgestorm.rpg.combat.MonsterVersusMonster;
import com.forgestorm.rpg.combat.MonsterVersusPlayer;
import com.forgestorm.rpg.combat.PlayerVersusMonster;
import com.forgestorm.rpg.combat.PlayerVersusPlayer;
...
public class EntityDamageByEntity implements Listener {
	...
	private void entityTester(LivingEntity damager, LivingEntity defender) {
		...
		if (!(defender instanceof ArmorStand)) {
			...
			if (damager instanceof Player && !(defender instanceof Player)) {
				...
				boolean isInMap = PLUGIN.getMonsterManager().getRpgEntity().containsKey(defender.getUniqueId());
	...
	private void startFatigue(Player damager) {
		PlayerProfile profile = PLUGIN.getPlayerManager().getPlayerProfile(damager);
